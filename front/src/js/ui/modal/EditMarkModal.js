...
class EditMarkModal extends MarkModal {
  ...
  _fillAttributes() {
    ...
    this._rating.updateRate(this._opts.rating - 1);
    if (this._opts.price) {
      this._pricing.updateRate(this._opts.price - 1);
    }
  ...
  submit(event) {
    if (super.submit(event)) {
      ...
      this._opts.rating = this._rating.currentRate + 1;
      if (this._rootElement.querySelector(`#nls-${this._opts.type}-price`)) {
        this._opts.price = this._pricing.currentRate + 1;
      }
