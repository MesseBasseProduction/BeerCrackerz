class ImageResizer {
  ...
  _events() {
    ...
    this._evtIds.push(window.Evts.addEvent('mouseup', this._dom.resizer, this._mouseUp, this));
    ...
    this._evtIds.push(window.Evts.addEvent('touchend', this._dom.resizer, this._mouseUp, this));
  }
  ...
  _mouseMove(event) {
    if (this._isGrabbed()) {
      ...
      if (this._grab.tl) { // Top/Left
        ...
        this._dom.resizer.style.top = `${offsetT + (offsetR - offsetX)}px`;
        ...
        if (this._pointer.x - offsetX > 0) { // Towards left
        ...
        if (this._pointer.y - offsetY > 0) { // Towards top
          // Blocking return condition, resizer is on an edge
