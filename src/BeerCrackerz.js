...
class BeerCrackerz extends MapHelper {
   ...
   * @description
	...
	_initMarkers() {
		return new Promise(resolve => {
			// Init map clusters for marks to be displayed
			this._clusters.spot = new window.L.MarkerClusterGroup();
			this._clusters.store = new window.L.MarkerClusterGroup();
			this._clusters.bar = new window.L.MarkerClusterGroup();
        ...
        this._map.addLayer(this._clusters.bar);
			...
			// Test data with minimal keys to work, will be retrieved from server when plugged
			const test = [{
				type: 'spot',
        ...
        this.markPopupFactory(test[i].type, test[i].name, test[i]).then(dom => {
          ...
          test[i].marker = this.placeMarker(test[i]);
					this._marks[test[i].type].push(test[i]);
					this._clusters[test[i].type].addLayer(test[i].marker);
				});
			}
   ...
   * @description
	...
	_markerSaved(options) {
		// Save marke in marks and clusters
